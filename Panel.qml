...
Panel {
  ...
  readonly property var projectOptions: {
    var rows = []
    var all = service ? service.projects : []
    for (var i = 0; i < all.length; i++) {
      var p = all[i]
      rows.push({ value: String(p.id), label: String(p.name || p.title || ("Project " + p.id)) })
    }
    return rows
  }
  ...
  readonly property var tasklistOptions: {
    var rows = []
    var all = service ? service.tasklists : []
    for (var i = 0; i < all.length; i++) {
      var t = all[i]
      rows.push({ value: String(t.id), label: String(t.name || t.title || ("Tasklist " + t.id)) })
    }
    return rows
  }
  ...
  function isTaskFinished(task) {
    ...
    if (task.state && typeof task.state === "object" && String(task.state.state || "").toLowerCase() === "finished") return true
    if (task.finished === true || task.is_finished === true) return true
    return false
  }

  function dueDateLabel(task) {
    var raw = String((task && (task.due_date || task.dueDate)) || "")
    if (raw === "") return ""
  ...
  readonly property var openTasks: {
    var all = service ? service.tasks : []
    var needle = String(taskFilter || "").trim().toLowerCase()
    var rows = []
    for (var i = 0; i < all.length; i++) {
      var task = all[i]
      if (isTaskFinished(task)) continue
      if (needle !== "" && String(task.name || task.title || "").toLowerCase().indexOf(needle) === -1) continue
      rows.push(task)
    }
    return rows
  }
  ...
  function priorityLabel(priority) {
    ...
    if (p === "h" || p === "high") return "H"
    if (p === "m" || p === "medium") return "M"
    if (p === "l" || p === "low") return "L"
    return ""
  }

  function priorityColor(priority) {
    var p = String(priority || "").toLowerCase()
    if (p === "h" || p === "high") return root.urgent
    return root.dim
  }
  ...
  function selectProject(id) {
    ...
    persistSettings({ selectedProjectId: id, selectedTasklistId: "" })
    if (service) service.refresh()
  }

  function selectTasklist(id) {
    persistSettings({ selectedTasklistId: id })
  }

  function submitQuickAdd() {
    var name = String(quickAddText || "").trim()
  ...
  function commitRename() {
    if (renamingTaskId === "" || !service) return
  ...
  function confirmDelete() {
    if (pendingDeleteId !== "" && service) service.deleteTask(pendingDeleteId)
    pendingDeleteId = ""
    pendingDeleteName = ""
  }
  ...
  KeyboardPanel {
    ...
    PanelKeyCatcher {
      ...
      onTextKey: function(text) {
        ...
        else if (text === "/") Qt.callLater(function() { filterField.forceActiveFocus() })
      }
      ...
      Flickable {
        ...
        Column {
          ...
          PanelHero {
            ...
            meta: {
              if (!service) return ""
              if (service.loading) return "Refreshing…"
              if (service.state !== "ready") return service.message
              if (service.tracking && service.tracking.active) {
                var name = String(service.tracking.taskName || service.tracking.task_name
          ...
          Button {
            visible: service && service.tracking && service.tracking.active
            width: parent.width
            text: "■  Stop tracking"
            bordered: true
            foreground: root.urgent
            fontFamily: root.fontFamily
            onClicked: if (service) service.stopTracking()
          }

          BorderSurface {
            visible: !service || service.state !== "ready"
            width: parent.width
            ...
            Text {
              ...
              text: service ? service.message : "Loading…"
              color: root.urgent
              font.family: root.fontFamily
          ...
          Dropdown {
            width: parent.width
            label: "PROJECT"
            value: service ? service.selectedProjectId : ""
            options: root.projectOptions
            foreground: root.foreground
            fontFamily: root.fontFamily
            onChanged: function(value) { root.selectProject(value) }
          }

          Dropdown {
            width: parent.width
            label: "TASKLIST (for quick-add)"
            value: service ? service.selectedTasklistId : ""
            options: root.tasklistOptions
            foreground: root.foreground
            fontFamily: root.fontFamily
            enabled: root.tasklistOptions.length > 0
            onChanged: function(value) { root.selectTasklist(value) }
          }
          ...
          RowLayout {
            ...
            TextField {
              ...
              enabled: service && service.selectedProjectId !== "" && service.selectedTasklistId !== ""
          ...
          Text {
            ...
            width: parent.width
            text: (service && service.selectedProjectId === "") ? "Pick a project above." : "No open tasks."
            color: root.dim
            font.family: root.fontFamily
            ...
            Repeater {
              ...
              CursorSurface {
                ...
                RowLayout {
                  ...
                  Text {
                    visible: root.priorityLabel(taskRow.modelData.priority) !== ""
                  ...
                  PanelActionButton {
                    ...
                    foreground: root.foreground
                    fontFamily: root.fontFamily
                    onClicked: if (service) service.startTracking(taskRow.modelData.id)
                  }
                  ...
                  PanelActionButton {
                    ...
                    foreground: root.foreground
                    fontFamily: root.fontFamily
                    onClicked: if (service) service.finishTask(taskRow.modelData.id)
                  }
                  ...
                  PanelActionButton {
                    ...
                    fontFamily: root.fontFamily
                    onClicked: {
                      if (root.renamingTaskId === String(taskRow.modelData.id)) root.cancelRename()
                      else root.beginRename(taskRow.modelData)
                    }
                  }
